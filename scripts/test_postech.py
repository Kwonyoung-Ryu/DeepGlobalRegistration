...
from dataloader.postech_loader import POSTECHPairDataset
...
from test_3dmatch import rte_rre
...
def evaluate(config, data_loader, method):
    ...
    stats = np.zeros((N, 4))  # bool succ, rte, rre, time, drive id

    stats_drive = np.empty((N, 1)).astype(str)
    ...
    for i in range(len(data_loader)):
        ...
        stats_drive[i, 0] = drive

        if stats[i, 0] == 0:
            logging.info(f"Failed with RTE: {stats[i, 1]}, RRE: {stats[i, 2]}")

        if i % 10 == 0:
            succ_rate, rte, rre, avg_time = stats[:i + 1].mean(0)
            logging.info(
    ...
    succ_rate, rte, rre, avg_time = stats.mean(0)
    logging.info(
    ...
    np.savez(out_file, stats=stats, stats_drive=stats_drive)
    analyze_stats(stats)
